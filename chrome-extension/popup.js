(function() {
  ...
  function scrollTable(tabId, position) {
    return new Promise((resolve) => {
      chrome.tabs.sendMessage(tabId, { action: 'scrollTable', position }, function(response) {
        if (chrome.runtime.lastError) {
          resolve(false);
        } else {
  ...
  function clickRefresh(tabId) {
    return new Promise((resolve) => {
      findAndClickRefreshButton(tabId, (success, error) => {
        resolve({ success, error });
      });
    });
  }

  async function clickRefreshWithRetry(tabId) {
    const result = await clickRefresh(tabId);
    if (result.success) return true;
    const errText = String(result.error || '');
    if (errText.includes('Receiving end does not exist')) {
      addLog('点击刷新按钮失败: ' + result.error, 'error');
      addLog('检测到内容脚本未连接，尝试注入...', 'error');
      const injected = await ensureContentScript(tabId);
      if (injected) {
        addLog('内容脚本已注入，重试点击刷新...');
        const retry = await clickRefresh(tabId);
        if (retry.success) {
          addLog('刷新按钮已点击');
          return true;
        }
        addLog('点击刷新按钮失败: ' + (retry.error || 'unknown'), 'error');
        return false;
      }
  ...
  async function performRefreshPasses(tabId) {
    const passes = ['left', 'center', 'right'];
    for (const position of passes) {
      const moved = await scrollTable(tabId, position);
      if (moved) {
        addLog('切换列区域: ' + position);
      } else {
        addLog('未找到可横向滚动区域，继续采集', 'error');
      }
      const clicked = await clickRefreshWithRetry(tabId);
      if (!clicked) continue;
      addLog('等待 10 秒采集数据...');
  ...
  collectBtn.addEventListener('click', async function() {
    ...
    try {
      ...
      const proceedAfterRefresh = async function() {
        await performRefreshPasses(tab.id);
        addLog('正在获取采集结果...');
      ...
      const clicked = await clickRefreshWithRetry(tab.id);
      if (clicked) {
        addLog('刷新按钮已点击');
      }
      proceedAfterRefresh();
